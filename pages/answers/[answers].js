...
export async function getServerSideProps(context) {
  ...
  getQuestions(assignmentId, teacherToken).then((openEndedCount) => {
    if (openEndedCount > 0) {
      getOpenEndedAnswer(openEndedCount, function (questionsAndAnswersJSON) {
        console.log(questionsAndAnswersJSON);
        return {
          props: {
            answers: questionsAndAnswersJSON,
          },
        };
      });
    } else {
